...
.deploy: &deploy
  ...
  tags:
    - deploy
  script:
    - npx -y -p shell-deployer@1.2.1 deploy-nodejs dist/spa/ --with-build
...
.deploy: &deploy
  ...
  tags:
    - deploy
  script:
    - npx --yes -p shell-deployer@1.2.1 deploy-laravel app.tgz --with-build
...
#### Node.js
Use the command `npx -y -p shell-deployer@1.2.1 build-nodejs` to build.

The build result is the directory specified by `npm run build`.

#### Laravel
Use the command `npx -y -p shell-deployer@1.2.1 build-laravel` to build.
